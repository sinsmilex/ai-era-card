...
- ✅ `GET /api/stats` без Bearer-секрета возвращает `401`; используйте
  ...
  документации, shell history или скриншотах.

**Следующие действия основателя:** опубликовать 0.1.4, выполнить dry-run'ы,
записать начальные stats, затем лично отправить готовый DM 5–10 подходящим
...
- [ ] Создать свежую полную founder-карточку и открыть её в инкогнито. У
  ...
  подставить **последнюю полную карточку**, если она была пересоздана.
  Проверить title, OG image, мобильный вид и ссылку на GitHub.
...
- [ ] Настроить доступ к `/api/stats` и записать начальные значения:
  `totalCards`, последние slugs и referer hosts. Не публиковать секрет
  авторизации и не вставлять его в скриншоты.
